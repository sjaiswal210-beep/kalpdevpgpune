...
import { BedDouble, User, Eye, Crown } from 'lucide-react';
import { ALL_ROOMS, PG_STRUCTURE, getRoomOccupancy, getRentForRoom, formatCurrency } from '../data/store';
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {PG_STRUCTURE[room.floor].label}
                    {isPremium && ' • Premium Room'}
                  </p>
                </div>
              ...
              <div className="space-y-3">
                {isPremium ? (
                  <BedCard label="Double Bed" tenant={bedA} />
                ) : (
                  <>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900 dark:text-white">{count}/{maxBeds} {isPremium ? 'Bed' : 'Beds'}</span>
                </div>
                <div className="mt-2 w-full h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full transition-all"
                    style={{ width: `${(count / maxBeds) * 100}%` }}
                  ></div>
                </div>
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                  Rent: {formatCurrency(getRentForRoom(room.number))}/month {isPremium ? '' : 'per bed'}
                </div>
              </div>
            </motion.div>
          );
    ...
    <div className={`flex items-center justify-between p-3 rounded-xl ${
      ...
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
          ...
          {tenant ? <User className="w-4 h-4 text-purple-600 dark:text-purple-300" /> : <BedDouble className="w-4 h-4 text-gray-500" />}
        </div>
        <div>
